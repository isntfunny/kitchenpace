...
3. **Navigation inkonsistent**: Minimap-Y (dagre-basiert) stimmt nicht mit Swipe-Reihenfolge (row-basiert) überein
4. **Hintergrund scheint durch**: Dark-Overlay zu transparent, Rezeptseite (Zutaten, Autor) bleibt lesbar
5. **"Schritt X / Y" irreführend**: Bei Graphen mit Branches gibt es keinen linearen Pfad
...
- **Kürzere Branches**: Steps werden zu Balken die über mehrere Spalten spannen
...
- **Fork/Merge-Erkennung**: Algorithmus identifiziert parallele Branch-Gruppen,
  berechnet max Schrittanzahl pro Gruppe, verteilt kürzere Branches
- **Zustände**: Done (grün), Current (orange + Glow), On-Path (dezentes Orange),
  Inactive (grau), Ghost (fast unsichtbar für andere Branches)
...
- Stärkerer Blur: `backdrop-filter: blur(20px+)`
- Höhere Opacity: `background: rgba(26,23,21,0.92+)`
...
3. Kürzere Branches mit `grid-column: span X` strecken
