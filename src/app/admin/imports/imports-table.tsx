            ...
            <Dialog.Portal>
                ...
                <Dialog.Content
                    className={css({
                        ...
                        overflow: 'hidden',
                        boxShadow: {
                            base: '0 25px 60px rgba(0,0,0,0.3)',
                            _dark: '0 25px 60px rgba(0,0,0,0.6)',
                        },
                    })}
                >
                    {/* Header */}
                    ...
                    >
                        <div
                            className={css({ display: 'flex', alignItems: 'center', gap: '1rem' })}
                        >
                    ...
                    >
                        ...
                        >
                            ...
                            <DetailField label="Kosten">
                                {formatCost(run.estimatedCostUsd)}
                            </DetailField>
                            <DetailField label="Markdown-Länge">
                                {run.markdownLength != null
                                    ? `${run.markdownLength.toLocaleString('de-DE')} Zeichen`
                                    : '—'}
                            </DetailField>
                        </div>
                        ...
                        >
                            ...
                            <DetailField label="Cached-Tokens">
                                <span className={css({ color: 'green.600', fontWeight: 500 })}>
                                    {formatTokens(run.cachedInputTokens)}
                                </span>
                            </DetailField>
                            ...
                            <DetailField label="Erstelltes Rezept">
                                ...
                                >
                                    <Link
                                        href={
                                            run.recipeSlug
                                                ? `/recipe/${run.recipeSlug}`
                                                : `/recipe/${run.recipeId}/edit`
                                        }
                                        target="_blank"
                                        className={css({
                                            color: 'primary',
                        ...
                        {(run.errorType || run.errorMessage) && (
                            ...
                            >
                                ...
                                {run.errorMessage && (
                                    <span className={css({ fontSize: 'sm', color: 'red.700' })}>
                                        {run.errorMessage}
                                    </span>
                                )}
                            </div>
            ...
            cell: ({ row }) =>
                row.original.sourceUrl ? (
                    ...
                    >
                        {(() => {
                            try {
                                return new URL(row.original.sourceUrl!).hostname;
                            } catch {
                                return row.original.sourceUrl;
                            }
                        })()}
                    </a>
                ) : (
            ...
            cell: ({ row }) =>
                row.original.recipeId ? (
                    ...
                    >
                        <Link
                            href={
                                row.original.recipeSlug
                                    ? `/recipe/${row.original.recipeSlug}`
                                    : `/recipe/${row.original.recipeId}/edit`
                            }
                            target="_blank"
                            className={css({
                                fontSize: 'base',
                        ...
                        <span
                            className={css({
                                ...
                                fontWeight: 600,
                                background:
                                    row.original.recipeStatus === 'PUBLISHED'
                                        ? 'green-100'
                                        : 'amber-100',
                                color:
                                    row.original.recipeStatus === 'PUBLISHED'
                        ...
                        >
                            {row.original.recipeStatus === 'PUBLISHED'
                                ? 'Veröffentlicht'
                                : row.original.recipeStatus === 'ARCHIVED'
                                  ? 'Archiviert'
                                  : 'Entwurf'}
                        </span>
                    </div>
            ...
            >
                ...
                >
                    ...
                    <div className={css({ display: 'flex', gap: '0.75rem' })}>
                        ...
                        <span
                            className={css({ fontSize: 'sm', color: 'muted', alignSelf: 'center' })}
                        >
